...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    description:
      "Fast typed System One decision from the local autonoxis server (Polaris 3 / polaris-3, Jev-compatible): choice, noul (P(true)), or score over a state. No text generation; returns probabilities and confidence.",
  ...
  pi.registerTool({
    ...
    description:
      "Next conductor action for a lane from Polaris 3 (polaris-3), using its trained convention (one `label` question over state {packet}). act is true when confidence >= 0.8, else \"escalate\".",
